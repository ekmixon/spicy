...
void Scope::insert(ID id, NodeRef&& n) {
    ...
    auto& nodes = _items[std::string(d.id())];
...
void Scope::insert(Declaration d, IntrusivePtr<Scope> scope) { insert(d.id(), std::move(d), std::move(scope)); }

void Scope::insert(ID id, Declaration d, IntrusivePtr<Scope> scope) {
    _nodes.push_back(std::move(d));

    if ( scope )
        _nodes.back().setScope(std::move(scope));
...
void Scope::initCanonicalIDs(const ID& parent) {
    for ( auto& [k, v] : items() ) {
        for ( auto& x : v ) {
            if ( ! x )
                continue;

            auto d = x->as<Declaration>();
            if ( ! d.canonicalID() ) {
                if ( parent )
                    d.setCanonicalID(ID(parent, d.id()));
                else
                    d.setCanonicalID(ID(d.id()));
            }
        }
    }
}
