...
public:
    ...
    /**
     * Returns a new result instance with its type replaced.
     *
     * @param d original result
     * @param t new type
     * @return new result that's equal to the original one but with its type replaced
     */
};
...
public:
    ...
    Function(function::Result result, const std::vector<function::Parameter>& params,
        ...
        : TypeBase(nodes(std::move(result), node::none, // TODO: Remove child 1
                         util::transform(params, [](const auto& p) { return Declaration(p); })),
                   std::move(m)),
          _flavor(flavor) {}

    const auto& result() const { return child<function::Result>(0); }
    auto parameters() const { return childs<function::Parameter>(2, -1); }
    ...
    auto _isResolved(ResolvedState* rstate) const {
        ...
        if ( ! type::isResolved(result().type(), rstate) )
            return false;

        for ( auto p = childs().begin() + 2; p != childs().end(); p++ ) {
