...
public:
    ...
    hilti::optional_ref<const type::unit::item::Field> field(const ID& id) const;
    /**
    ...
    bool operator==(const Unit& other) const { return false; } // FIXMEE typeID() == other.typeID(); }
    ...
    static void setSelf(Node* n) {
        ...
        n->childs()[0] = Declaration(Unit::_self(*n, n->meta()));
    }

private:
    static Declaration _self(Node& n, const Meta& m) {
        Expression self = hilti::expression::Keyword(hilti::expression::keyword::Kind::Self, NodeRef(n), m);
        // TODO: Why the two groupings here? (This is copied from type::Struct).
        return hilti::declaration::Expression("self",
                                              hilti::expression::Grouping(hilti::expression::Grouping(std::move(self))),
